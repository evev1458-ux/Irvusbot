...
# Flask (Alchemy Webhook) için ayrı bir kanal
@app.route('/webhook', methods=['POST'])
def webhook():
    data = request.json
    if data: Thread(target=send_buy, args=(data,)).start()
    return "ok", 200
...
if __name__ == "__main__":
    # Webhook'u arka planda başlat
    Thread(target=run_flask).start()
    
    # Botu yeni token ile başlat
    ...
    # Çakışmayı önlemek için drop_pending_updates=True şart!
