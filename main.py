...
@app.route('/')
def home():
    return "IRVUS SIMPLE BOT ONLINE", 200
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/start ve /star komutları için butonlu karşılama"""
    msg = (f"💎 **IRVUS TOKEN**\n\n"
...
async def ciz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt:
        return await update.message.reply_text("❌ Kullanım: `/ciz bir aslan` ")
    
    await update.message.reply_text("🎨 Irvus AI çiziyor, lütfen bekleyin...")
    
    # Pollinations AI kullanarak görsel oluşturma
    image_url = f"https://image.pollinations.ai/prompt/{quote(prompt)}?seed={int(time.time())}"
    
    await update.message.reply_photo(
        photo=image_url, 
        caption=f"🖼 **Sonuç:** `{prompt}`"
    )

# --- 5. ÇALIŞTIRICI ---
def run_web():
    # Render'ın PORT değişkenini kullanıyoruz
...
if __name__ == "__main__":
    ...
    print(">>> IRVUS SADE BOT AKTİF")
    
    # Botu çalıştır ve birikmiş mesajları temizle
