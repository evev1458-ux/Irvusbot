...
async def fiyat(u, c):
    ...
    except: await u.message.reply_text("⚠️ Fiyat verisi alınamadı.")

async def ask(u, c):
    query = " ".join(c.args)
    if not query: return await u.message.reply_text("Lütfen bir soru yazın.")
    
    await u.message.chat.send_action("typing")
    # İngilizce hatasını çözen Llama-3 modeli
    url = f"https://text.pollinations.ai/{quote(query)}?model=llama&system=YouAreIrvusAI"
    
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=15) as r:
                ans = await r.text()
                # Eğer hala hata mesajı gelirse alternatif model (mistral) dene
                if "IMPORTANT NOTICE" in ans:
                    async with s.get(f"https://text.pollinations.ai/{quote(query)}?model=mistral") as r2:
                        ans = await r2.text()
                await u.message.reply_text(f"🤖 **Irvus AI:**\n\n{ans}")
    except: await u.message.reply_text("⚠️ AI şu an meşgul.")

async def draw(u, c):
    p = " ".join(c.args)
    if not p: return await u.message.reply_text("Ne çizelim?")
    m = await u.message.reply_text("🎨 Görsel hazırlanıyor...")
    ...
    try:
        ...
        await m.delete()
    except: await m.edit_text("⚠️ Görsel oluşturulamadı.")
...
if __name__ == "__main__":
    ...
    # Botu yeni token ile tekil olarak başlat
    bot = ApplicationBuilder().token(TOKEN).build()
    
    bot.add_handler(CommandHandler("start", start))
    bot.add_handler(CommandHandler(["fiyat", "price"], fiyat))
    bot.add_handler(CommandHandler(["sor", "ask"], ask))
    bot.add_handler(CommandHandler(["ciz", "draw"], draw))
    
    # Çakışmaları temizle ve botu dinlemeye al
